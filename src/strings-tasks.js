...
function removeFirstOccurrences(str, value) {
  ...
  return i >= 0
    ? str.slice(0, i) + str.slice(i + value.length, str.length)
    : str;
}
...
function countVowels(str) {
  const str2 = 'aeiouyYAEIOU';
  let count = 0;

  for (let i = 0; i < str.length; i += 1) {
    if (str2.indexOf(str[i]) !== -1) count += 1;
  }
...
function isPalindrome(str) {
  const str1 = str.toLowerCase().replace(/[^А-яЁёA-Za-z]/g, '');
...
function invertCase(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    if (str1[i] === str[i]) str1[i] = str1[i].toUpperCase();
  }
...
function getCardId(value) {
  ...
  const cardArr = [...value];
  const suitValue = cardSuits.get(cardArr.pop());
  const rangValue = cardArr.length > 1 ? 9 : cardRangs.indexOf(cardArr[0]);

  return suitValue + rangValue;
}
